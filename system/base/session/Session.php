...
Class Session
{
	...
	{
		$SessionConf = \System\Config::instance()->get('SessionConf');
		self::name(\System\Config::instance()->check($SessionConf,'SESSION_NAME'));
		self::savePath(\System\Config::instance()->check($SessionConf,'SESSION_SAVEPATH'));
		self::useCookies(\System\Config::instance()->check($SessionConf,'SESSION_USE_COOKIES'));
		self::cacheExpire(\System\Config::instance()->check($SessionConf,'SESSION_CACHE_EXPIRE'));
		self::cacheLimiter(\System\Config::instance()->check($SessionConf,'SESSION_CACHE_LIMITER'));
		self::gcProbability(\System\Config::instance()->check($SessionConf,'SESSION_GC_PROBABILITY'));
		self::maxLifetime(\System\Config::instance()->check($SessionConf,'SESSION_MAX_LIFETIME'));
		self::prefix(\System\Config::instance()->check($SessionConf,'SESSION_PREFIX'));
		$saveHandler = \System\Config::instance()->check($SessionConf,'SESSION_SAVE_HANDLER','files');
		self::saveHandler($saveHandler);
		if('user' === $saveHandler)
		{
			self::userSaveHandler(\System\Config::instance()->check($SessionConf,'SESSION_USER_SAVE_HANDLER'));
		}
		session_start();
	}
