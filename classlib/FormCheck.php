...
class FormCheck extends \System\Singleton
{
    /**验证手机号码格式
     * @param $mobile
     * @return mixed
     ...
     * @param $mobile
     * @param $code
     * @param $codeType 1:注册 2:换绑 3:忘记密码
     * @return mixed
     */
    public function checkSmsCode($mobile, $smscode, $codeType = 1){
        ...
        } elseif ($codeArr->time + 1800 < dateFormat()) {  //验证码30分钟内有效
     ...
     * @return mixed
     */
    public function checkUserSmsCode($mobile, $smscode, $codeType = 1){

        $codeArr = Cache::get("userMobileSmsCode");
        if (!$codeArr) {
            return $this->result("验证码不存在");
        } elseif ($codeArr->time + 1800 < dateFormat()) {  //验证码30分钟内有效
            Cache::forget("userMobileSmsCode");  //清除验证码信息
            return $this->result("验证码已过期");
        } elseif ($codeArr->type != $codeType) {
            return $this->result("验证码不存在");
        } else {
            if ($mobile != $codeArr->mobile) {
                ...
                Cache::forget("userMobileSmsCode");
                return $this->result("验证通过", 1);
            }
        }
    }
     ...
     * @return object|void
     */
    public function checkCode($code, $length = 6){

        $pattern = '/^\d{' . $length . '}$/';

        if(!preg_match($pattern, $code)){

            return $this->result("验证码不合法");
        }else{
     ...
     * @return object|void
     */
    public function checkChinese($str, $begin = 2, $end = 10){

        if (preg_match('/^([\xe4-\xe9][\x80-\xbf]{2}){'.$begin.','.$end.'}$/', $str)) {
            $msg = "验证通过";
            $code = 1;
        } else {
            $msg = "验证不合法";
